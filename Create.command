...
tui_template() {
	case "$2" in
		"SKIP") printf "\x1B[1;97;100m %s \x1B[0;1;2m \x1B[1m%s \x1B[0;2m%s (%s)\x1B[0m\n" "-" "Template:" "$1" "Skipped" ;;
...
for template in "${HERE}/Create/Templates"/*.iconset; do
	...
	({
		...
		# Check if the template is enabled by default.
		if LC_ALL=C type "template_enabled" 2>/dev/null | grep 'function' &>/dev/null; then
			if ! template_enabled; then
				tui_template "$template_name" SKIP
				exit 1
			fi
		fi
